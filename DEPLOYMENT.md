...
```bash
# Build the image
docker build -t nulltracer-renderer server/

# Run via docker-compose (renderer only)
docker-compose up -d
```

Or create the container manually via the Unraid Docker tab using the template at [`nulltracer-renderer.xml`](assets/nulltracer-renderer.xml). Key settings:
...
|----------------|-----------|------|
| `/srv/nulltracer` | `/path/to/nulltracer` | Read-Only |
