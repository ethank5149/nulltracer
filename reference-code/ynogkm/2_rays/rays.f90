...
!*****************************************************************************************************
      ...
      velocity(1)=0.0D0 
      velocity(2)=0.0D0 
      velocity(3)=0.8D0!*exppsi/expnu*(-somega)!*zero 
      ...
      DO i=0,m-1 
          Do j=0,n-1
              vptl(3)= velo*cos(j*two*pi/(n-1))*sin(i*pi/(m-1))
              vptl(1)= velo*sin(j*two*pi/(n-1))*sin(i*pi/(m-1))
              vptl(2)= velo*cos(i*pi/(m-1))
              ...
              call metricg(r_ini,sin_ini,cos_ini,a_spin,e,somega,expnu,exppsi,expmu1,expmu2)
              !velocity(1)=0.0D0!expmu1/expnu*r_ini/(r_ini**(2.05D0/two)+a_spin)!*zero
              !velocity(2)=0.0D0!expmu2/expnu/(r_ini**(three/two)+a_spin)*zero
              !velocity(3)=exppsi/expnu*(-somiga)*1.d0  !exppsi/expnu*(one/(r_ini**(three/two)+a_spin)-somiga) 
