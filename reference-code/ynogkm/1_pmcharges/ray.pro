...
ln=801ul
 ...
 Openr,xcoor,'rayx1.txt',/Get_Lun
  Point_lun,xcoor,0
  xcoord2=fltarr(ln,nm)
  ReadF,xcoor,xcoord2
  
   Openr,ycoor,'rayy1.txt',/Get_Lun
  Point_lun,ycoor,0
  ycoord2=fltarr(ln,nm)
  ReadF,ycoor,ycoord2 
  ...
  ratio=1;3.529
   ...
   ;xoff=(LL-xxss)/2.,yoff=(3*LL/2.-yyss)/2.,
    !p.font = 0
   device,filename='rays1.ps',xsize=xxss,ysize=yyss,bits_per_pixel=8,/color,xoff=(21.5-xxss)/2.0,yoff=(30-yyss)/2.,$
...
; loadct,30
 ...
 ;plot,[0,1.5],[0,1.1],pos=[(xxss/(2*Lc))*1000,(yyss/(2*Lc))*1000,$
  ...
  ;         /device,/ynozero,charsize=1,charthick=3;,yticklen=0.001;,yminor=5  
	...
	my=2 ;figure numbers on vertical direction.
   ...
   for ny=0,my-1 do begin	
		...
		plot,[-4,6],[-4,6],pos=[pos_llft+deltax*nx+deltay*ny],xrange=[-3,7],yrange=[-4,4],$
		...
		If ny eq 1 and nx eq 0 then begin
		    for i=0,nm-1 do begin 
			oplot,xcoord2(*,i),ycoord2(*,i),color=249
		    endfor
		endif 
;************************************************************
                ;this part to draw a out circle of the disk 
                if ny eq 0 then begin
	            diskl=1.2615339366124405
                endif
