...
if not keyword_set(bkg) then bkg='white' else bkg=bkg(0)
 ...
 Openr,xcoor,'xx2.txt',/Get_Lun
  Point_lun,xcoor,0
  xcoord=fltarr(ln,nm)
  ReadF,xcoor,xcoord
  
   Openr,ycoor,'yy2.txt',/Get_Lun
  Point_lun,ycoor,0
  ycoord=fltarr(ln,nm)
  ReadF,ycoor,ycoord
  
   Openr,zcoor,'zz2.txt',/Get_Lun
  Point_lun,zcoor,0
  zcoord=fltarr(ln,nm)
  ReadF,zcoor,zcoord
...
loadct,33  ;  loadct,3 also looks nice too
    ...
    l=16  & xxss=l & yyss=l
    !p.font = 0
    device,filename='./tital_6.ps',color=1,xsize=xxss,ysize=yyss,$
   ...
   for ny=0,my-1 do begin	
		...
		plot,[-length,length],[-length,length],pos=[pos_llft+deltax*nx+deltay*ny],xrange=[-5,25],yrange=[-10,10],$
...
;^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	plots,disklx,diskly,color=1b,thick=4b 
  
        for i=nmstart,nm-1 do begin
  
            x1=[xcoord[*,i],xcoord[0,i]]*cp-[ycoord[*,i],ycoord[0,i]]*sp
            y1=-[xcoord[*,i],xcoord[0,i]]*sp+[ycoord[*,i],ycoord[0,i]]*cp
            z1=[zcoord[*,i],zcoord[0,i]]
