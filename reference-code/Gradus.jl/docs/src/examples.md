...
Alternatively, using [Makie.jl](https://github.com/MakieOrg/Makie.jl) as the
plotting backend:

```julia
using Makie, CairoMakie
# second argument is the number of points to sample
all_points = extract_path.(sols.u, 1024)
...
# plot a bounding circle for the event horizon
x, y, z = Gradus.boundingsphere(
    Gradus.inner_radius(m);
    azimuth = ax.azimuth.val,
    elevation = ax.elevation.val
)
lines!(ax, x, y, z; color = :black, linewidth = 2.0)

# plot all the geodesics
...
fig
```
![](./example-figures/example-3d-makie.svg)
...
## Time-averaged emissivity profiles

See also [Emissivity and illumination profiles](@ref).
...
# from here it's plotting
fig = Figure()
ax = Axis(fig[1, 1], xscale = log10, yscale = log10, ylabel = "ε", xlabel = "r")
xlims!(ax, 1, 200)
for (model, prof) in zip(models, profiles)
    em = emissivity_at(prof, radii)
    mask = @. em > 0
    lines!(ax, radii[mask], em[mask], color = model.r, colorrange = (1, 10))
end
fig
```
