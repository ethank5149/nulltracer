...
- `OutOfDomain`: left the integration chart by travelling to effective infinity
  (see [`chart_for_metric`](@ref)).
- `WithinInnerBoundary`: left the integration chart by falling into the inner
  horizon [`inner_radius`](@ref).
- `IntersectedWithGeometry`: geodesics intersected with
  [`AbstractAccretionGeometry`](@ref) and terminated there. Note that this status
  code only applies to optically thick geometry (see [`optical_property`](@ref)
  for more).
- `NoStatus`: the default status code, which is used to imply no calculation
...
Base.broadcastable(x::AbstractMetric) = Ref(x)
...
Return a tuple with each non-zero metric component for the metric described by
`m` at position `x`. Note that the position need not be a four-vector, and for
specific implementations may only be a subset of the total manifold
coordinates. See specific implementations for subtypes of
...
Calculate the four-acceleration of the geodesic equation for a spacetime given
by the metric `m`, four-position `x` and four-velocity `v`.

A geodesic is the shortest path connecting two points in space. For flat space,
this is just a straight line. In curved space, geodesics are analogous to
straight lines between points (e.g. the great circle on a sphere).

The geodesic equation calculates the acceleration experienced by a particle at
position ``x^\\mu = (t, r, \\theta, \\phi)`` travelling with tangential
velocity ``v^\\nu = \\text{d} x / \\text{d} \\lambda`` due to the curvature of
spacetime. The curvature is calculated from the metric, encoded in the
[Christoffel symbols](https://en.wikipedia.org/wiki/Christoffel_symbols). The
acceleration is then calculated via
```math
...
```
where ``\\Gamma^{\\mu}_{\\phantom{\\mu}\\nu\\sigma}`` are the Christoffel
symbols (of the second kind), and ``\\lambda`` is an affine parameter
...
Calculate the time component ``v^t`` of a velocity vector `v`, which would
constrain the vector at a position `x` as a geodesic with invariant mass `μ`.

The velocity vector needs to only specify the ``v^r``, ``v^\\theta``, and
``v^\\phi`` component, as the ``v^t`` is constrained in GR by
```math
g_{\\sigma\\nu} v^\\sigma v^\\nu = -\\mu^2,
```
where ``\\mu^2`` is the invariant mass of the particle. This furthermore
permits a choice of geodesic to trace. The choices correspond to
...
Return the innermost valid coordinate relative to the origin, for use in
geodesic tracing.

This usually represents some property of the metric, e.g. event horizon radius
in Kerr/Schwarzschild metrics, or throat diameter in worm hole metrics.
"""
...
Return the [`AbstractMetric`](@ref) type associated with the metric parameters
`m`.
"""
...
Numerically evaluate the corresponding metric for [`AbstractMetric`](@ref),
given parameter values `m` and some point `u`.
"""
...
Parameters that are constant throughout the integration (e.g. mass or
frequency) for any number of geodesics. Also used to dispatch different tracing
problems.
"""
...
Parameters that are made available at each step of the integration, that need
not be constant.  For example, the turning points or withing-geometry flags.
...
Update (mutate) the `old` integration parameters to take the value of the
`new`. Function should return the `old`.

Note this function is practically only used to update any mutable fields in the
integration parameters, such as resetting any changes to an original state.
"""
...
Supertype for geodesic points, used to store information about specific points
along geodesic trajectories.

!!! note
    Currently limited to storing the start and endpoint of any given
    trajectory. To keep the full geodesic path, it is encouraged to use the
    `SciMLBase.AbstractODESolution` directly.

Must minimally have the same fields as [`GeodesicPoint`](@ref).  Examples
include [`Gradus.FirstOrderGeodesicPoint`](@ref).
"""
...
Supertype of all accretion geometry. Concrete sub-types must minimally
implement:
- [`in_nearby_region`](@ref)
- [`has_intersect`](@ref)

Alternativey, for more control, either [`intersects_geometry`](@ref) or
[`geometry_collision_callback`](@ref) may be implemented for a given geometry
type.

Geometry intersection calculations are performed by strapping discrete
callbacks to the integration procedure.
"""
...
Base.broadcastable(x::AbstractAccretionGeometry) = Ref(x)
...
Supertype for axis-symmetric geometry that are specified by a height
cross-section function. Subtypes are required to implement
[`cross_section`](@ref).
"""
...
"""
    AbstractEmissivityProfile

Abstract type for binning structures over discs.
"""
abstract type AbstractEmissivityProfile end

Base.broadcastable(x::AbstractEmissivityProfile) = Ref(x)

emissivity_at(prof::AbstractEmissivityProfile, ::AbstractGeodesicPoint) =
    error("Not implemented for $(typeof(prof))")

coordtime_at(prof::AbstractEmissivityProfile, ::AbstractGeodesicPoint) =
    error("Not implemented for $(typeof(prof))")

emissivity_at(prof::AbstractEmissivityProfile, v::AbstractArray) =
    map(i -> emissivity_at(prof, i), v)
coordtime_at(prof::AbstractEmissivityProfile, v::AbstractArray) =
...
Note that [`sample_position_velocity`](@ref) has a number of its own
requirements (see that function's documentation). This function must be
implemented as a fallback for other methods.

If special symmetries exist, these may be used in the implementations of
higher-order functions, such as [`emissivity_profile`](@ref).
"""
...
Base.broadcastable(x::AbstractCoronaModel) = Ref(x)

"""
    abstract type AbstractLocalSpectrum end

Abstract type specifying interface for spectra in the local frame of an
emitter.
"""
abstract type AbstractLocalSpectrum end

Base.broadcastable(x::AbstractLocalSpectrum) = Ref(x)

local_spectrum(spectrum::AbstractLocalSpectrum, g) =
...
The transfer function approach is often faster and converges to higher
numerical accuracy than the other methods. They can also be pre-computed for
use in spectral models for fitting.
"""
...
include("corona/lamp-post.jl")
include("corona/extended.jl")
include("corona/ring.jl")
