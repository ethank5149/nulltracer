...
"""
    extract_path(sol, n_points; kwargs...)

Extract the `(x, y, z)` points of a geodesic solution that was traced with
`save_on = true`. `sol` should be an ODE solution. If used with an
`EnsembleSolution`, call as `extract_path.(sols.u, n_points)`.

The indented use of this function is to pull out information for plotting the
geodesic.

The points are interpolated around a particular point `p` of the geodesic. `p`
is the endpoint of the geodesic when its [`StatusCodes`](@ref) is
`IntersectedWithGeometry` or `WithinInnerBoundary`. Otherwise it is the
midpoint along the whole integrated trajectory.

- `n_points` is the number of points to use to construct the path. It is
  therefore also the length of the vectors `x`, `y` and `z`.

Keyword arguments:

- `t_span = 30.0`, the affine time window around `p` to interpolate over.

# Example

```
sol = tracegeodesics(m, x_init, v_init, 1000.0; save_on = true)
# interpolate 1024 points
x, y, z = extract_path(sol, 1024)
```
"""
extract_path(sol, n_points; kwargs...) = _extract_path(sol, n_points; kwargs...)
export extract_path
...
"""
    boundingsphere(R; azimuth = 0, elevation = 0)

Constructs the `(x, y, z)` points to plot the outline of a sphere (a "bounding
sphere") with a given radius. The azimuth and elevation should be set to the
azimuth and elevation at which the camera in the 3D scene would be looking at
the origin.
...
x, y, z = Gradus.boundingsphere(
    1.0;
    azimuth = ax.azimuth.val,
    elevation = ax.elevation.val
)
lines!(ax, x, y, z)
...
```
"""
function boundingsphere(R; azimuth = 0, elevation = 0)
    phi_circ = 0.0:0.001:2π
    x = @. R * cos(phi_circ)
    y = @. R * sin(phi_circ)
    z = zeros(length(y))

    t = rotate_z(azimuth - π) * rotate_y(elevation - π / 2)
    points = reduce(hcat, [x, y, z])'
    translated = reduce(hcat, map(p -> t * p, eachcol(points)))

    (; x = translated[1, :], y = translated[2, :], z = translated[3, :])
end
