...
A structure that represents the start and end point along a geodesic. To obtain
a `GeodesicPoint`, use [`unpack_solution`](@ref) or
[`unpack_solution_full`](@ref) on the result of a [`tracegeodesics`](@ref).

Fields:
- `status::StatusCodes.T`: A [`StatusCodes`](@ref) code of the integrator for
  this geodesic.
