...
radial_component(setup::IntegrationSetup{T,<:AbstractEmissivityProfile}, r) where {T} =
    emissivity_at(setup.profile, r)

time_component(setup::IntegrationSetup, r) = setup.profile.time(r) - setup.t0
time_component(setup::IntegrationSetup{T,<:AbstractEmissivityProfile}, r) where {T} =
    coordtime_at(setup.profile, r) - setup.t0

integration_setup(prof::AbstractEmissivityProfile, transfer_functions; kwargs...) =
...
"""
    integrate_lineprofile(
        prof::AbstractEmissivityProfile,
        transfer_functions,
        g_grid;
        kwargs...
    )
...
[`transferfunctions`](@ref)) with either an [`AbstractEmissivityProfile`](@ref) or
