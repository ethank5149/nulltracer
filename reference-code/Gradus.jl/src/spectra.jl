"""
struct PowerLawSpectrum{T} <: AbstractLocalSpectrum
    Γ::T
end

Spectrum for the power law simply returns the photon index `Γ`. `I` stores the `local_spectrum` which is a function 
...
"""
    function local_spectrum(spectrum::PowerLawSpectrum, g)
