 ...
 *
 *  Note: All Butcher tableau coefficients are now double precision,
 *  unlike the GLSL version which used float32 rationals.
 * ============================================================ */
...
/* Helper macro: call geoRHS and store results in kr_N, kth_N, etc. */
#define GEORHS_STAGE(N, R_EXPR, TH_EXPR, PR_EXPR, PTH_EXPR) \
    geoRHS((R_EXPR), (TH_EXPR), (PR_EXPR), (PTH_EXPR), a, b, Q2, \
           &kr##N, &kth##N, &kphi##N, &kpr##N, &kpth##N)


extern "C" __global__
void trace_rkdp8(const RenderParams p, unsigned char *output) {
    ...
    for (int i = 0; i < p.steps; i++) {
        ...
        r   += he*(bw1*kr1+bw6*kr6+bw7*kr7+bw8*kr8+bw9*kr9+bw10*kr10+bw11*kr11+bw12*kr12+bw13*kr13);
        th  += he*(bw1*kth1+bw6*kth6+bw7*kth7+bw8*kth8+bw9*kth9+bw10*kth10+bw11*kth11+bw12*kth12+bw13*kth13);
        phi += he*(bw1*kphi1+bw6*kphi6+bw7*kphi7+bw8*kphi8+bw9*kphi9+bw10*kphi10+bw11*kphi11+bw12*kphi12+bw13*kphi13);
        pr  += he*(bw1*kpr1+bw6*kpr6+bw7*kpr7+bw8*kpr8+bw9*kpr9+bw10*kpr10+bw11*kpr11+bw12*kpr12+bw13*kpr13);
        pth += he*(bw1*kpth1+bw6*kpth6+bw7*kpth7+bw8*kpth8+bw9*kpth9+bw10*kpth10+bw11*kpth11+bw12*kpth12+bw13*kpth13);
